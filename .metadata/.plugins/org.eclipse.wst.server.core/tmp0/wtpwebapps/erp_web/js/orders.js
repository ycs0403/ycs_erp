$(function(){
	...
	if(Request['oper']=='myorders'){
		if(Request['type']*1==1){
			url="orders_myListByPage?t1.type=1";//type:1、采购
		...
		if(Request['type']*1 == 2){
			url = "orders_myListByPage?t1.type=2&t1.state=0";//type:2、销售 State:0、未审核
	...
	$('#grid').datagrid({
		...
		columns:[[
			...
			onDblClickRow:function(rowIndex, rowData){
				...
				$('#type').html(formatDate(rowData.type));
				...
				if(rowData.state*1==1&&rowData.type*1==2){
					...
					toolbar.push({
						...
						handler:function(){
							...
							$('#waybillGrid').datagird({
								url:'orders_waybilldetailList?waybillSn='+$('#waybillSn').html(),
								columns:[[
									{field:'exedata',title:'执行日期',width:100},
